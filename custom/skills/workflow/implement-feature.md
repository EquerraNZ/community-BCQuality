...
- **Tests per acceptance criterion** - one or more tests per acceptance criterion in the spec, using `ai-test-driven-development` for Copilot/agent features. A missing test for an acceptance criterion is a finding.
...
- **Docs and roadmap** - feature docs updated (and `bc-extension-test-guide` re-run if a TEST_GUIDE is maintained), roadmap item set to `done`. A stale doc or roadmap status is a finding.
